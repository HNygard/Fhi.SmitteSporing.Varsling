...
export function nationalidValidator(): ValidatorFn {
  return (control: AbstractControl): { [key: string]: any } | null => {
    const valid = validateNorwegianIdNumber(control.value);
    return !valid ? { 'nationalid': { value: control.value } } : null;
  };
}

@Directive({
  selector: '[nationalid]',
  providers: [{ provide: NG_VALIDATORS, useExisting: NationalidValidatorDirective, multi: true }]
})
export class NationalidValidatorDirective implements Validator {
  validate(control: AbstractControl): { [key: string]: any } | null {
    return control.value ? nationalidValidator()(control)
      : null;
  }
}
